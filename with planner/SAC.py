...
def first_network_train(subgoal):
    ...
    policy_net1,policy_optimizer1,frame_idx1,rewards1 = load_checkpoint(policy_net1,policy_optimizer1,'resumeTrain500000fr1')
    print("Resuming from {}".format(frame_idx1))
    while frame_idx1 < max_frames:
        state = env1.reset()
        print(state)
        episode_reward = 0
        ...
        for step in range(max_steps):
            ...
            if done:
                break
        state = {'epoch': frame_idx1 + 1, 'state_dict': policy_net1.state_dict(),
                    'optimizer': policy_optimizer1.state_dict(), 'rewards': rewards1}
        torch.save(state, 'resumeTrain500000fr1')
        start_episode+=1

        print(" \r first network : frame {} reward: {}".format(frame_idx1,episode_reward))
        rewards1.append(episode_reward)
    print(frame_idx1)
    # print(rewards1)
    plot(frame_idx1, rewards1)
...
def second_network_train(subgoal):
    ...
    policy_net2,policy_optimizer2,frame_idx2,rewards2 = load_checkpoint(policy_net2,policy_optimizer2,'resumeTrain500000fr2')
    print("Resuming from {}".format(frame_idx2))
    while frame_idx2 < max_frames:
        state = env2.reset()
        print(state)
        episode_reward = 0
        for step in range(max_steps):
            ...
            if done:
                break
        state = {'epoch': frame_idx2 + 1, 'state_dict': policy_net2.state_dict(),
                    'optimizer': policy_optimizer2.state_dict(), 'rewards': rewards2}
        torch.save(state, 'resumeTrain500000fr2')
        start_episode+=1

        print("\r second network : frame {} reward: {}".format(frame_idx2,episode_reward))
        rewards2.append(episode_reward)
    print(frame_idx2)
    # print(rewards2)
    plot(frame_idx2, rewards2)
...
if __name__ == '__main__':
    ...
    frame_idx=frame_idx1 + frame_idx2
