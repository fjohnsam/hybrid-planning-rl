...
void print_op_name_infile(FILE* fp, int index )
...

{
  ...
  for ( i = 0; i < gnum_plan_ops; i++ ) {
    ...
    print_op_name_infile(fp, gplan_ops[i] );
  ...
  /*for ( i = 0; i < gnum_fl_conn; i++ ) {
    for (j = 0; j < 10 ; j++) {
      printf("%d:%f ", j, gfl_conn[i].level[j]);
    }
    printf("\nPC\n");
    for ( j = 0 ; j < gfl_conn[i].num_PC ; j++) {
      printf("%d:%d ", j, gfl_conn[i].PC[j]);
    }
    printf("\ncur_max=%f\n", gfl_conn[i].curr_max_assigned);
    printf("\nlevel \n");
  }
  */
